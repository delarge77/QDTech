...
- (void)testShouldReturnBadRequestWhen4xxStatusCodeIsReturned {
    ...
    NSURLSessionTask *task = [QDTechUserDetailConnectionController loadUserDetailWithSessionOption:@"posts?userId=1" withCompletionHandler:^(NSArray *response, NSError *error) {
...
- (void)testShouldBeAbleToParseAndReturnSessions {
    ...
    NSURLSessionTask *task = [QDTechUserDetailConnectionController loadUserDetailWithSessionOption:@"posts?userId=1" withCompletionHandler:^(NSArray *response, NSError *error) {
        XCTAssertNil(error, @"Should be able to details sessions without an error");
        XCTAssertEqual(10, response.count, @"should be able to parse all 10 details inside the fixture");
